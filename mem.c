...
* the my_realloc function behaves like the my_malloc function for the
* specified size. Otherwise, if ptr does not match a pointer earlier returned by
* the my_calloc. my_malloc, or my_realloc function, or if the space has been
...
/**************************************************************************************************************************************
* Module		: defragment_my_heap()
*
* Description	: This function combines all the allocated memory chunks into contiguous
* space without losing any of the values stored in them.
*
**************************************************************************************************************************************/
...
defragment_my_heap(void)
{
	...
	while(traverse->next != NULL)
	{
		if(!(traverse->free))
		{
			...
			if (free_space > 0)
			{
				memmove((void*)traverse + sizeof(NODE) + traverse->size, (void*)traverse->next, (traverse->next->size + sizeof(NODE)));
				traverse->next = (void*)traverse + sizeof(NODE) + traverse->size;
			}
		...
		if(traverse->free)
		{
			memmove((void*)traverse,(void*)traverse->next,(traverse->next->size + sizeof(NODE)));
			continue;
		}
	...
	memset((void*)traverse,0, -(long)traverse +((long)StaticMemory + HEAP_SIZE)); //Clear rest of memory
}
